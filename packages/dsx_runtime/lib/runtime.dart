...
class Signal<T> {
  ...
  Signal(this._value);
  T call() { _track(this); return _value; }
  void set(T v) {
    if (v == _value) return;
    _value = v;
...
void effect(EffectFn fn) {
  ...
  _stack.add(c);
  try { runner(); } finally { _stack.removeLast(); }
}

abstract class NodeLike {}
class TextNode extends NodeLike { final String text; TextNode(this.text); }
...
class DynNode extends NodeLike { final String Function() getter; DynNode(this.getter); }

NodeLike text(String t) => TextNode(t);
NodeLike dyn(String Function() getter) => DynNode(getter);
NodeLike h(String tag, [Map<String, dynamic>? props, List<NodeLike>? children]) =>
...
void _render(NodeLike n, Node parent) {
  if (n is TextNode) {
    ...
    for (final c in n.children) { _render(c, el); }
    parent.append(el);
